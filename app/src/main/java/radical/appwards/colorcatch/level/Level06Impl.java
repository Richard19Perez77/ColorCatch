...
/**
 * A class that defines radical.appwards.colorcatch.level 6 radical.appwards.colorcatch.logic.
 * 
 * @author Rick
 * 
 */

public class Level06Impl implements Level {
	...
	public void myDraw(Canvas canvas) {
		canvas.drawText("Level 6", gv.screenW / 2, gv.whitePaint.getTextSize(),
		...
		if (gv.gameTimer < gv.INTRO_PAUSE_TIME) {
			gv.cyclePaint();
			canvas.drawText("!! Loading Level 6 !!", gv.screenW / 2,
	...
	/**
	 * Initializes the radical.appwards.colorcatch.level and then begins creatign enemies.
	 */
	@Override
	public void updatePhysics(Context context) {
		...
		if (!gv.gameVarsAreLoading) {
			...
			if (gv.getEnemyCreation()
					&& gv.gameTimer % (createEnemyTimer * 2) == 0) {
				gl.getNewMovables();
			}
